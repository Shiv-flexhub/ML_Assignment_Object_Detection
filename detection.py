...
def make_prediction(net, layerNames, image, confidenceThreshold, nmsThreshold):
    ...
    boxes, confidences, classIds = extract_boxes_confidences_classids(
        outputs, confidenceThreshold, height, width)
...
def detect_objects(nnWeights, nnConfiguration, labelsPath, confidenceThreshold, nmsThreshold, imagePath, videoPath):
    ...
    if imagePath is not None:
        ...
        boxes, confidences, classIds, ids = make_prediction(
            net, layerNames, image, confidenceThreshold, nmsThreshold)
        image = draw_bounding_boxes(
            image, boxes, confidences, classIds, ids, labels, colors)
        ...
        fileName = str(os.path.basename(
            os.path.abspath(videoPath))).split(".")[0] + ".mp4"
        output = cv2.VideoWriter("output/{}".format(fileName),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Script to detect objects using YOLOv4")
    parser.add_argument("-w", "--nnWeights", type=str,
                        default="weights/yolov4-tiny-bdd100k_best.weights", help="Path to neural network weights")
    parser.add_argument("-cfg", "--nnConfiguration", type=str,
                        default="config/yolov4-tiny-bdd100k.cfg", help="Path to neural network configuration")
    parser.add_argument("-l", "--labelsPath", type=str,
                        default="data/bdd100k.names", help="Path to labels file")
    parser.add_argument("-c", "--confidenceThreshold", type=float,
                        default=0.5, help="Minimum confidence required to detect objects")
    parser.add_argument("-n", "--nmsThreshold", type=float, default=0.3,
                        help="Minimum threshold required for Non-Maximum Suppression")

    inputPath = parser.add_mutually_exclusive_group()
    inputPath.add_argument("-i", "--imagePath", type=str,
                           default=None, help="Path to the input image file")
    inputPath.add_argument("-v", "--videoPath", type=str,
                           default=None, help="Path to the input video file")

    args = parser.parse_args()

    detect_objects(args.nnWeights, args.nnConfiguration, args.labelsPath,
                   args.confidenceThreshold, args.nmsThreshold, args.imagePath, args.videoPath)
